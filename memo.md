...
### バージョニング
- セマンティックバージョニング
1. メジャーバージョン: 後方互換性のない大きな変更 (X.Y.ZのXの部分)
2. マイナーバージョン: 後方互換性のある機能追加や改善 (X.Y.ZのYの部分)
3. パッチバージョン: 後方互換性があり、機能追加のない修正 (X.Y.ZのZの部分)

パッチバージョンリリースでも時によってはフタを開けると大きな影響があったりと欠点もあるにはある、それを差し引いてもベストプラクティス
...
## アナウンス
リリースノートの活用 (Github Releases)
- 破壊的変更: "Breaking Changes"みたいなタイトルで丁寧に告知する
- 新機能の追加: "New Features"みたいなタイトルで知らせるのが吉
- 既存機能の改善: "Enhancement"みたいなタイトルで知らせるのが吉
- 不具合修正: 情報が細かくなりやすいのでリリースノートに含めないのも策、"Bug Fixes"みたいなタイトル
...
## クラウドプロバイダのクレデンシャル
### 静的クレデンシャル
パスワードやアクセスキー、長期運用を想定。
コード内での平文埋め込みは死刑、Actionsのsecretsとかで運用保存
欠点は長命、万一漏洩すると被害が拡大しやすい。
